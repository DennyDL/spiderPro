...
string Url::getGet(string str_url)
{
    string str1("/"),str("");
	...
	if(str_url[n] == '\0')
        ...
        return str_url.substr(n,string::npos);
}
...
string Url::getFileType(string str_url)
{
	...
	int n2 = str_url.find("?",0);

	int n = str_url.rfind(".",n2);
	if(n == string::npos)
	{
		return str;
	}

	return str_url.substr(n+1,n2-n-1);
}
